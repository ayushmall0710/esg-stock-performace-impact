...
from src.analysis.regression_models import (
    run_rq1_sharpe_esg,
    run_rq2_volatility_esg,
    run_rq3_pillars,
)
...
def main():
    ...
    try:
        ...
        all_results["rq1"] = {"model": model_rq1, "results": results_rq1}

        # Save results
        with open(output_dir / "rq1_results.txt", "w") as f:
            f.write(str(model_rq1.summary()))

        print(f"\n[SAVED] Results saved to: {output_dir / 'rq1_results.txt'}")

    except Exception as e:
        print(f"\n[ERROR] Error in RQ1: {e}")
        import traceback
    ...
    try:
        ...
        all_results["rq2"] = {"model": model_rq2, "results": results_rq2}

        # Save results
        with open(output_dir / "rq2_results.txt", "w") as f:
            f.write(str(model_rq2.summary()))

        print(f"\n[SAVED] Results saved to: {output_dir / 'rq2_results.txt'}")

    except Exception as e:
        print(f"\n[ERROR] Error in RQ2: {e}")
        import traceback
    ...
    try:
        ...
        all_results["rq3"] = {"models": models_rq3, "results": results_rq3}
    ...
    if "rq1" in all_results:
        try:
            ...
            print(f"\n[WARNING] Error running diagnostics for RQ1: {e}")
    ...
    if "rq2" in all_results:
        try:
            ...
            print(f"\n[WARNING] Error running diagnostics for RQ2: {e}")
    ...
    if "rq1" in all_results:
        ...
        print("\n[INFO] RQ1: ESG Score → Sharpe Ratio")
        ...
        print(
            f"\tInterpretation: {'Higher ESG → Higher risk-adjusted returns' if r['esg_coef'] > 0 else 'Higher ESG → Lower risk-adjusted returns'}"
        )

    if "rq2" in all_results:
        r = all_results["rq2"]["results"]
        print("\n[INFO] RQ2: ESG Score → Volatility")
        print(f"\tCoefficient: {r['esg_coef']:.6f}")
        print(f"\tP-value: {r['esg_pvalue']:.4f}")
        print(f"\tSignificant: {'[YES]' if r['esg_significant'] else '[NO]'}")
        print(f"\tR²: {r['r_squared']:.4f}")
        print(
            f"\tInterpretation: {'Higher ESG → Lower volatility (more stable)' if r['esg_coef'] < 0 else 'Higher ESG → Higher volatility (less stable)'}"
        )

    if "rq3" in all_results:
        r = all_results["rq3"]["results"]
        if "sharpe" in r:
            print("\n[INFO] RQ3a: ESG Pillars → Sharpe Ratio")
            print(
                f"\tE coefficient: {r['sharpe']['e_coef']:.6f} (p={r['sharpe']['e_pvalue']:.4f})"
            )
            print(
                f"\tS coefficient: {r['sharpe']['s_coef']:.6f} (p={r['sharpe']['s_pvalue']:.4f})"
            )
            print(
                f"\tG coefficient: {r['sharpe']['g_coef']:.6f} (p={r['sharpe']['g_pvalue']:.4f})"
            )
            print(f"\tDominant pillar: {r['sharpe']['dominant_pillar']}")

        if "volatility" in r:
            print("\n[INFO] RQ3b: ESG Pillars → Volatility")
            print(
                f"\tE coefficient: {r['volatility']['e_coef']:.6f} (p={r['volatility']['e_pvalue']:.4f})"
            )
            print(
                f"\tS coefficient: {r['volatility']['s_coef']:.6f} (p={r['volatility']['s_pvalue']:.4f})"
            )
            print(
                f"\tG coefficient: {r['volatility']['g_coef']:.6f} (p={r['volatility']['g_pvalue']:.4f})"
            )
