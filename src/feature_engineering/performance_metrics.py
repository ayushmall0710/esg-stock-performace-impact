...
def calculate_performance_metrics(df: pd.DataFrame,
                                   ...
                                   excess_return_col: str = 'Excess_Return') -> pd.DataFrame:
    """
    ...
    for ticker in df[ticker_col].unique():
        ...
        if len(ticker_data) < 200:  # Require at least 200 trading days (~8 months)
            print(f"   ⚠️  Skipping {ticker}: only {len(ticker_data)} trading days")
            continue
        ...
        if std_excess_return > 0:
            metrics['Sharpe_Ratio'] = (mean_excess_return / std_excess_return) * np.sqrt(252)
        else:
        ...
        metrics['Annualized_Return'] = (1 + metrics['Cumulative_Return']) ** (252 / n_days) - 1
    ...
    print(f"\n✅ Calculated metrics for {len(performance_df)} tickers")
    ...
    # Display sample
    print("\n📊 Sample results:")
    display_cols = ['Ticker', 'Sharpe_Ratio', 'Annualized_Excess_Return', 'Cumulative_Return', 'Trading_Days']
