...
def download_fred_data(
    ...
    if fred_api_key and fred_api_key != "your_fred_api_key_here":
        ...
        try:
            ...
            print(f"[INFO] Records downloaded: {len(df)}")
            print(f"[STATS] Date range: {df.index.min()} to {df.index.max()}")
            ...
            print(f"\n[ERROR] Error fetching data from FRED API: {e}")
            print("\nFalling back to manual download instructions...")
            fred_api_key = None

    # Option B: Manual download instructions
    if not fred_api_key or fred_api_key == "your_fred_api_key_here":
        print("\n[INFO] FRED API key not configured. Manual download required.")
        ...
        if output_file.exists():
            print(f"\n[OK] Found existing file: {output_file}")
            try:
            ...
            print(f"\n[WARNING] File not found: {output_file}")
