...
def process_risk_free_rate(
    input_file: str = "data/raw/DGS3MO.csv",
    output_file: str = "data/processed/risk_free_rate.csv",
    trading_days_file: str = "data/processed/returns.csv",
) -> pd.DataFrame:
    """
    ...
    if not date_col or not rate_col:
        ...
        if len(df.columns) >= 2:
            ...
            print(f"[WARNING]  Guessing columns: {date_col} (date), {rate_col} (rate)")
        else:
            print("[ERROR] Could not identify date and rate columns")
            return None
    ...
    try:
        ...
        if date_col_returns:
            returns_df[date_col_returns] = pd.to_datetime(
                returns_df[date_col_returns], utc=True
            )
            trading_days = returns_df[date_col_returns].unique()
            trading_days = pd.Series(trading_days).sort_values()
            print(f"[OK] Found {len(trading_days)} unique trading days")
        else:
            print("[WARNING]  Could not find date column in returns data")
            trading_days = None
    except FileNotFoundError:
        print("[WARNING]  Trading days file not found, using all dates from FRED")
        trading_days = None

    # Align with trading days
    if trading_days is not None:
        print("\n[PROCESSING] Aligning risk-free rate with trading days...")
    ...
    # Statistics
    print("\n[STATS] Risk-Free Rate Statistics:")
    print(
        f"\tAnnual rate range: {df['Annual_Rate'].min():.4f}% - {df['Annual_Rate'].max():.4f}%"
    )
    print(f"\tMean annual rate: {df['Annual_Rate'].mean():.4f}%")
    print(
        f"\tMean daily rate: {df['Daily_RF_Rate'].mean():.6f} ({df['Daily_RF_Rate'].mean() * 100:.4f}%)"
    )

    # Annualized daily rate (for verification)
    annualized_from_daily = (1 + df["Daily_RF_Rate"].mean()) ** 252 - 1
    print(
        f"\tAnnualized daily rate: {annualized_from_daily * 100:.4f}% (should match annual rate)"
    )

    # Check for any remaining NaN
    remaining_nan = df["Daily_RF_Rate"].isnull().sum()
    if remaining_nan > 0:
        print(f"\n[WARNING]  Warning: {remaining_nan} dates still have NaN values")
        print("\tDropping these dates...")
